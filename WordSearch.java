...
public class WordSearch {
    ...
    private int solveDirection( int baseRow, int baseCol, int rowDelta, int colDelta ) {
        ...
        // Begin iterating from the cell next to the starting cell in the specified direction.
        for (int i = baseRow + rowDelta, j = baseCol + colDelta;
             i >= 0 && j >= 0 && i < rows && j < columns; // Ensure we stay within the words 4 by 4.
             i += rowDelta, j += colDelta) { // Move to the next cell in the specified direction.
            ...
            if (theWords[searchResult].equals(charSequence.toString())) {
                numMatches++;
                System.out.println("Found " + charSequence + " at " + baseRow + " " + baseCol + " to "
                + i + " " + j);
            }
